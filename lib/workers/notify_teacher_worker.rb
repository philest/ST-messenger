...
class NotifyTeacherWorker
  ...
  def new_users_notification(teacher)
    ...
    puts teacher.signature, count, family, list_o_names, quicklink

    new_users_notification_helper(teacher.signature, count, family, list_o_names, quicklink)
  end

  def new_users_notification_helper(sig, count, family, list_o_names, quicklink)
    # Authenticate with your API key
    auth = { :api_key => ENV['CREATESEND_API_KEY'] }
    # The unique identifier for this smart email
