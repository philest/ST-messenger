...
class ScheduleWorker
  ...
  def within_time_range(user, range, acceptable_days = [1, 3])
  	# TODO: ensure that Time.now is in UTC time

  	# server timein UTC
		# now 			= Time.now.utc.seconds_since_midnight
    now       = Time.now.utc
    puts "now in seconds since midnight = #{now}"
    ...
    puts "user utc in seconds since = #{user_sendtime_utc}"
		user_day 	 = get_local_day(Time.now.utc, user)
    puts "user_day PLEEEEASAAAAASSEE = #{user_day}" 

    valid_for_user = acceptable_days.include?(user_day)
    puts "valid_for_user = #{valid_for_user}"
  ...
  def adjust_tz(user)
    ...
    new_time = Time.new(Time.now.year, Time.now.month, Time.now.day, ast.hour, ast.min, ast.sec, ast.utc_offset)
